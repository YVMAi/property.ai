...
import { Download, FileText, ShieldCheck, StickyNote, ArrowLeft, Pencil } from 'lucide-react';
...
export default function TenantViewPage() {
  ...
  const { id } = useParams();
  const { getTenantById, runBGV, updateNotes } = useTenantsContext();
  ...
  if (!tenant) {
    return (
      <div className="flex flex-col items-center justify-center py-20 animate-fade-in">
        <p className="text-lg text-muted-foreground mb-4">Tenant not found.</p>
        <Button variant="outline" onClick={() => navigate('/users/tenants')}>
  ...
  return <TenantViewContent tenant={tenant} />;
}

function TenantViewContent({ tenant }: { tenant: NonNullable<ReturnType<ReturnType<typeof useTenantsContext>['getTenantById']>> }) {
  const navigate = useNavigate();
  const { runBGV, updateNotes } = useTenantsContext();
  const { toast } = useToast();
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => navigate('/users/tenants')} className="shrink-0">
            <ArrowLeft className="h-5 w-5" />
          </Button>
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-semibold text-foreground">{getTenantDisplayName(tenant)}</h1>
              {getCategoryBadge()}
            </div>
            <p className="text-sm text-muted-foreground mt-1">{tenant.email} · {tenant.phone || 'No phone'}</p>
          </div>
        </div>
        <Button variant="outline" onClick={() => navigate(`/users/tenants/${tenant.id}/edit`)}>
          ...
          <Card className="border-border/50">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                ...
                <Button size="sm" variant="outline" onClick={handleRunBGV}>
                  Run BGV
                </Button>
              </div>
            </CardHeader>
          ...
          <Card className="border-border/50">
            ...
            <CardContent>
              <Textarea
                ...
                rows={5}
              />
              <Button size="sm" className="mt-3 btn-primary" onClick={handleSaveNotes}>
                Save Notes
              </Button>
            </CardContent>
          </Card>
